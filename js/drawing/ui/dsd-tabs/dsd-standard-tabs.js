...
window.DSDStandardTabs = (() => {
  ...
  function renderStyleTab(d) {
    d.style = d.style || {};
    const s = d.style;
    const defaultStyle = (window.DrawingManager && DrawingManager._getToolStyle) ? DrawingManager._getToolStyle(d.tool) : {};
    const caps = window.DrawingSettingsDialog.getCaps(d.tool);

    const color      = s.color || defaultStyle.color || '#2962ff';
    ...
    const fillColor  = s.fillColor || defaultStyle.fillColor || 'rgba(41,98,255,0.1)';
    ...
    if (d.tool === 'flattopbottom') {
      ...
      const priceColor = s.priceColor || '#2962ff';
      ...
      <div class="dsd-row-controls">
        ...
        <div class="dsd-cap-drop" style="position:relative;margin-right:4px;">
          <div class="dsd-cap-drop-btn" data-side="left"
               style="display:flex;align-items:center;gap:6px;background:#131722;border:1px solid #7d808b;border-radius:4px;height:30px;padding:0 8px;cursor:pointer;color:#a3a6af;">
  ...
  `;
    ...
    if (d.tool === 'channel') {
      ...
      const levelsHtml = channelLevels.map((lvl, i) => {
        ...
        <div class="dsd-row-inline" style="align-items:center; gap:8px; margin-bottom:8px; ${dimStyle}">
          ...
          <input type="number" class="dsd-input js-ch-level-val" data-idx="${i}" value="${lvl.v}" step="0.001" ${isEdge ? 'readonly' : ''} style="width:80px; flex:none; height:30px; box-sizing:border-box; padding:2px 6px; text-align:center;">
      ...
      <div class="dsd-row dsd-row-check">
        ...
        <div class="dsd-row-controls" style="margin-left:28px;">
    ...
    if (caps.capArrows) {
      ...
      html += `
          <div class="dsd-cap-drop" style="position:relative;margin-right:4px;">
            <div class="dsd-cap-drop-btn" data-side="left" style="display:flex;align-items:center;gap:6px;background:#131722;border:1px solid #7d808b;border-radius:4px;height:30px;padding:0 8px;cursor:pointer;color:#a3a6af;">
              <span class="dsd-cap-icon" style="transform:scaleX(-1);">${capLabels[capLeft] || capLabels.normal}</span>
              <svg width="10" height="10" viewBox="0 0 10 10" style="flex-shrink:0;color:#787b86;"><path d="M2 3.5l3 3 3-3" stroke="currentColor" fill="none" stroke-width="1.5"/></svg>
            </div>
          </div>
          <div class="dsd-cap-drop" style="position:relative;">
            <div class="dsd-cap-drop-btn" data-side="right" style="display:flex;align-items:center;gap:6px;background:#131722;border:1px solid #7d808b;border-radius:4px;height:30px;padding:0 8px;cursor:pointer;color:#a3a6af;">
    ...
    if (caps.extend) {
      if (d.tool === 'vline') {
        // vline: tek bir Extend checkbox — indikatör penceresi üzerinden geçip zaman cetbeline kadar uzatır
        const extendAll = s.extendAll !== undefined ? !!s.extendAll : (extRight || extLeft || false);
        html += `
      <div class="dsd-row dsd-row-check">
        <label class="dsd-checkbox-label">
          <input type="checkbox" id="dsd-ext-all" ${extendAll?'checked':''}>
          Extend
        </label>
      </div>`;
      } else {
    ...
    if (caps.hasTimeLabel) {
      const timeLabel = s.timeLabel !== false;
      html += `
      <div class="dsd-row dsd-row-check">
        <label class="dsd-checkbox-label">
          <input type="checkbox" id="dsd-timelabel" ${timeLabel?'checked':''}>
          Time label
        </label>
      </div>`;
    }
  ...
  function renderTextTab(d) {
    const s = d.style || {};
    const defaultStyle = (window.DrawingManager && DrawingManager._getToolStyle) ? DrawingManager._getToolStyle(d.tool) : {};
    const textColor = s.textColor || defaultStyle.textColor || '#ffffff';
    ...
    const showBg     = s.showBg !== false;
    const fillColor  = s.fillColor || defaultStyle.fillColor || 'rgba(41,98,255,0.2)';
    ...
    return `
      ...
      <div class="dsd-row" style="margin-bottom:16px;">
        <textarea class="dsd-textarea" id="dsd-text" placeholder="Add text" style="min-height:140px; border-radius:6px; border:1px solid #7d808b; background:#131722; color:#a3a6af; padding:8px;">${text}</textarea>
      </div>
      ...
      ${d.tool === 'vline' ? `
      <div class="dsd-row" style="margin-top:12px;">
        <label class="dsd-label">Text orientation</label>
        <div style="display:flex; gap:8px;">
          <select class="dsd-select" id="dsd-textOrientation">
