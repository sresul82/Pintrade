    ...
    if (d.tool === 'channel') {
      ...
      const levelsHtml = channelLevels.map((lvl, i) => {
        ...
        <div class="dsd-row-inline" style="align-items:center; gap:8px; margin-bottom:8px; ${dimStyle}">
          ...
          <input type="number" class="dsd-input js-ch-level-val" data-idx="${i}" value="${lvl.v}" step="0.001" ${isEdge ? 'readonly' : ''} style="width:80px; height:30px; box-sizing:border-box; padding:2px 6px; border:1px solid #2a2e39; border-radius:4px; text-align:center;">
